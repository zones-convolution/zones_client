        ...
        for (int i = 0; i < numChannels; ++i)
        {
            ...
            if (curWritePosition + numSamples > bufferSize - 1)
            {
                ...
                audioBuffer->copyFrom (
                    i, curWritePosition, newAudioData, i, startSample, samplesToEdgeOfBuffer);

                audioBuffer->copyFrom (i,
                                       0,
                                       newAudioData,
                                       i,
            ...
            {
                audioBuffer->copyFrom (
                    i, curWritePosition, newAudioData, i, startSample, numSamples);
            }
        }
    ...
    void readSamples (juce::AudioBuffer<Type> & bufferToFill, int readSize)
    {
        ...
        for (int i = 0; i < numChannels; ++i)
        {
            ...
            if (readPosition + readSize > bufferSize - 1)
            {
                ...
                bufferToFill.copyFrom (i, 0, *audioBuffer, i, readPosition, samplesToEdgeOfBuffer);

                bufferToFill.copyFrom (
                    i, samplesToEdgeOfBuffer, *audioBuffer, i, 0, readSize - samplesToEdgeOfBuffer);
            }
            // If we stay inside the ring
            else
            {
